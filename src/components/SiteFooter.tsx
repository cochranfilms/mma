import Link from 'next/link';

const footerNavigation = {
  services: [
    { name: 'Media Relations', href: '/services#media-relations' },
    { name: 'Web Presence', href: '/services#web-presence' },
    { name: 'Photo & Printing', href: '/services#photo-printing' },
    { name: 'Content & Campaigns', href: '/services#content-campaigns' },
    { name: 'Partnerships', href: '/services#partnership-development' },
    { name: 'Brand Strategy', href: '/services#brand-strategy' },
  ],
  company: [
    { name: 'About', href: '/about' },
    { name: 'Case Studies', href: '/work' },
    { name: 'Contact', href: '/contact' },
    { name: 'Privacy', href: '/privacy' },
    { name: 'Accessibility', href: '/accessibility' },
    { name: 'Terms', href: '/terms' },
  ],
  resources: [
    { name: 'Blog', href: '/blog' },
    { name: 'Resources', href: '/resources' },
    { name: 'FAQ', href: '/faq' },
    { name: 'Newsletter', href: '/newsletter' },
  ],
};

export default function SiteFooter() {
  return (
    <footer className="bg-accent-900 text-white">
      <div className="container-custom py-16">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {/* Company Info */}
          <div className="lg:col-span-1">
            <div className="flex items-center space-x-2 mb-4">
              <div className="w-8 h-8 bg-accent-600 rounded-lg flex items-center justify-center">
                <span className="text-white font-bold text-sm">MMA</span>
              </div>
              <span className="font-bold text-xl">Marketing Mousetrap</span>
            </div>
            <p className="text-accent-300 text-sm leading-relaxed mb-6">
              B2B media connections that compound your brand's reach. We upgrade your web presence, content, and partnerships—then turn it into pipeline.
            </p>
            <div className="space-y-2 text-sm">
              <p className="text-accent-300">
                <span className="font-medium text-white">Email:</span> hello@marketingmousetrap.com
              </p>
              <p className="text-accent-300">
                <span className="font-medium text-white">Phone:</span> +1-555-123-4567
              </p>
            </div>
          </div>

          {/* Services */}
          <div>
            <h3 className="font-semibold text-lg mb-4">Services</h3>
            <ul className="space-y-2">
              {footerNavigation.services.map((item) => (
                <li key={item.name}>
                  <Link
                    href={item.href}
                    className="text-accent-300 hover:text-white transition-colors text-sm"
                  >
                    {item.name}
                  </Link>
                </li>
              ))}
            </ul>
          </div>

          {/* Company */}
          <div>
            <h3 className="font-semibold text-lg mb-4">Company</h3>
            <ul className="space-y-2">
              {footerNavigation.company.map((item) => (
                <li key={item.name}>
                  <Link
                    href={item.href}
                    className="text-accent-300 hover:text-white transition-colors text-sm"
                  >
                    {item.name}
                  </Link>
                </li>
              ))}
            </ul>
          </div>

          {/* Resources & CTA */}
          <div>
            <h3 className="font-semibold text-lg mb-4">Resources</h3>
            <ul className="space-y-2 mb-6">
              {footerNavigation.resources.map((item) => (
                <li key={item.name}>
                  <Link
                    href={item.href}
                    className="text-accent-300 hover:text-white transition-colors text-sm"
                  >
                    {item.name}
                  </Link>
                </li>
              ))}
            </ul>
            
            <div className="bg-accent-800 rounded-lg p-4">
              <h4 className="font-medium text-sm mb-2">Ready to get started?</h4>
              <p className="text-accent-300 text-xs mb-3">
                Book a free consultation to discuss your B2B marketing needs.
              </p>
              <Link
                href="/contact"
                className="block w-full bg-accent-600 hover:bg-accent-700 text-white text-center py-2 px-4 rounded-lg font-medium text-sm transition-colors"
              >
                Book Consultation
              </Link>
            </div>
          </div>
        </div>

        {/* Bottom Bar */}
        <div className="border-t border-accent-800 mt-12 pt-8">
          <div className="flex flex-col md:flex-row justify-between items-center space-y-4 md:space-y-0">
            <p className="text-accent-400 text-sm">
              © {new Date().getFullYear()} Marketing Mousetrap Agency. All rights reserved.
            </p>
            
            <div className="flex items-center space-x-6">
              <Link
                href="https://linkedin.com/company/marketingmousetrap"
                className="text-accent-400 hover:text-white transition-colors"
                aria-label="LinkedIn"
              >
                <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/>
                </svg>
              </Link>
              
              <Link
                href="https://twitter.com/marketingmousetrap"
                className="text-accent-400 hover:text-white transition-colors"
                aria-label="Twitter"
              >
                <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M23.953 4.57a10 10 0 01-2.825.775 4.958 4.958 0 002.163-2.723c-.951.555-2.005.959-3.127 1.184a4.92 4.92 0 00-8.384 4.482C7.69 8.095 4.067 6.13 1.64 3.162a4.822 4.822 0 00-.666 2.475c0 1.71.87 3.213 2.188 4.096a4.904 4.904 0 01-2.228-.616v.06a4.923 4.923 0 003.946 4.827 4.996 4.996 0 01-2.212.085 4.936 4.936 0 004.604 3.417 9.867 9.867 0 01-6.102 2.105c-.39 0-.779-.023-1.17-.067a13.995 13.995 0 007.557 2.209c9.053 0 13.998-7.496 13.998-13.985 0-.21 0-.42-.015-.63A9.935 9.935 0 0024 4.59z"/>
                </svg>
              </Link>
            </div>
          </div>
        </div>
      </div>
    </footer>
  );
}
